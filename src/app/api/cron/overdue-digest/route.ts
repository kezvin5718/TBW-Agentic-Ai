  ...
  try {
    ...
    if (!overdueTasks || overdueTasks.length === 0) {
      return NextResponse.json({
        success: true,
        message: "No overdue tasks detected. Everything is on schedule!",
      });
    }
    ...
    // Reads founder phone number from environment (defaulting to a simulator fallback if not set)
    ...
    // 4. Also store this digest in approvals log or system alerts log
    return NextResponse.json({
      success: true,
