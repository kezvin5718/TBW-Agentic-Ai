--
-- tbw-os Supabase Database Schema
-- Run this in your Supabase SQL Editor to configure the complete role-based schema.
--
...
-- Clients can only see their own brand record. Founder and employee see all.
...
-- Client Credentials Table (Encrypted Meta Graph Tokens & IG Business IDs)
...
-- Add media_plan JSONB column to monthly_plans
ALTER TABLE public.monthly_plans ADD COLUMN IF NOT EXISTS media_plan JSONB DEFAULT '{}'::jsonb;

-- Create Ad Ops Audit table for tracking Meta API writes
CREATE TABLE IF NOT EXISTS public.ad_ops_audit (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  client_id UUID REFERENCES public.clients(id) ON DELETE CASCADE NOT NULL,
  campaign_id UUID REFERENCES public.campaigns(id) ON DELETE SET NULL,
  action_type TEXT NOT NULL, -- 'create_campaign', 'create_adset', 'create_ad', 'update_budget', 'activate_campaign'
  platform TEXT NOT NULL DEFAULT 'meta',
  payload JSONB DEFAULT '{}'::jsonb,
  response JSONB DEFAULT '{}'::jsonb,
  status TEXT NOT NULL DEFAULT 'success' CHECK (status IN ('success', 'failed')),
  actor_role TEXT NOT NULL, -- 'system', 'founder'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- Add optimisation_rules JSONB column dynamically
...
DROP POLICY IF EXISTS "Employee and Client read prompt templates" ON public.prompt_templates;
...
-- 6. Knowledge Import Audit logging
...
-- 7. Agency Brain (Shared anonymized learnings)
